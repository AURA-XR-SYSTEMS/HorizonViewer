...
export default function AdminExportPanel({
  ...
  return (
    <aside
      ...
      className="admin-panel absolute top-4 right-4 z-[70] flex max-h-[calc(100%-2rem)] w-[min(440px,calc(100%-2rem))] flex-col overflow-hidden rounded-[var(--radius-panel)] border border-white/25 bg-black/78 text-left text-white shadow-2xl backdrop-blur-xl"
    >
      <div className="border-b border-white/10 px-4 py-3">
        ...
        <p className="mt-2 text-sm leading-5 text-white/72">
          Mimics the Unreal export flow using the currently available backend export-job
          endpoints.
        </p>
      </div>
      ...
      <div className="admin-panel-scroll flex-1 space-y-4 overflow-y-auto px-4 py-4">
        <section className="space-y-3">
          <label className="block text-xs font-medium tracking-[0.18em] text-white/55 uppercase">
            Workspace ID
          </label>
          <input
            data-testid="workspace-input"
            className="w-full rounded-xl border border-white/15 bg-white/8 px-3 py-2 text-sm text-white transition outline-none focus:border-sky-300/60 focus:bg-white/12"
            value={workspaceId}
        ...
        <section className="space-y-3 rounded-2xl border border-white/10 bg-white/4 p-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <span
              ...
              className="rounded-full border border-white/10 bg-white/6 px-2 py-1 text-xs text-white/72"
            >
              {job?.status ?? 'none'}
            </span>
          </div>
          <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-sm text-white/78">
            ...
            <dd data-testid="job-export-id" className="font-mono text-[12px] break-all">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <label className="flex items-center gap-2 text-sm text-white/72">
              <input
                ...
                checked={autoPoll}
                onChange={(event) =>
                  setAutoPoll((event.target as HTMLInputElement).checked)
                }
              />
              Auto-poll
            </label>
        ...
        {lastError ? (
          <section
            ...
            className="rounded-2xl border border-rose-400/30 bg-rose-500/14 p-3 text-sm text-rose-100"
          >
