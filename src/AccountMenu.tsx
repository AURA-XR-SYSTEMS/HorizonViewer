 ...
 *
 * The dropdown is portalled to the body rather than rendered in place. The
 * header bar it sits in is a fixed-height strip with overflow hidden, so an
 * in-place panel is clipped to nothing and the button looks dead — no z-index
 * escapes a clipping ancestor. Portalling also keeps the panel from being
 * squeezed by the bar's flex layout.
 */
...
const AccountMenu: React.FC<AccountMenuProps> = ({ session, currentExportId }) => {
  ...
  // The panel is fixed-positioned, so it has to be told where the button is.
  ...
  // Dismiss on an outside click, matching the other header popovers. The panel
  // is portalled out of this subtree, so it needs checking separately or every
  // click inside it would count as "outside" and close the menu.
  useEffect(() => {
    if (!open) return;
          ...
          >
            {restoring ? (
              ...
              <form onSubmit={handleSignIn} className="flex flex-col gap-2">
                ...
                <div
                  style={{ fontSize: 11, color: 'rgba(25,25,25,0.5)', marginBottom: 2 }}
                >
